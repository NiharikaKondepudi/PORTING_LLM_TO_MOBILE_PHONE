...
try:
    # Get the time string and remove the 'Z' to make it "naive"
    start_str_naive = event["start"]["dateTime"].replace("Z", "")
    end_str_naive = event["end"]["dateTime"].replace("Z", "")

    # Parse the naive string
    start_dt_naive = datetime.fromisoformat(start_str_naive)
    end_dt_naive = datetime.fromisoformat(end_str_naive)

    # Localize the naive time to your timezone
...
try:
    ...
    print("⚠ Intent failed. Try opening the calendar manually.")
